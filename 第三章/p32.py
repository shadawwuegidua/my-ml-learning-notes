...
# === dot 的两种等价用法 ===
# 1. 函数式调用：np.dot(a, b)
#    它是 numpy 模块级的函数，如果传入的是普通的列表，NumPy 会自动将它们转换为数组进行就算。
print("\n--- 函数式调用 np.dot(t1, t3) ---")
print(np.dot(t1, t3))

# 2. 方法式调用：a.dot(b)
#    这是 ndarray 对象自带的一个方法，调用者（比如这里的 t1）必须已经是一个 numpy 数组。
#    这种写法在面向对象编程时更清晰，也省去了敲 'np.' 的麻烦。
print("\n--- 方法式调用 t1.dot(t3) ---")
print(t1.dot(t3))

# 另外，Python 3.5 以后引入了专用的矩阵乘法运算符 `@`，
# 它等价于 dot (针对二维矩阵)，写法最简单：
print("\n--- 使用 @ 运算符 t1 @ t3 ---")
print(t1 @ t3)
